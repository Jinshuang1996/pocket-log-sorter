...
- 支持从 Finder 拖入整个应用窗口，也支持把素材卡片拖到 Finder 或其他剪辑软件。
- 窗口支持自由拖拽缩放和 macOS 原生全屏，紧凑窗口与宽屏会自动调整侧栏、预览区和导出区尺寸。
...
- 上半部分播放器和导出控制保持固定；下方横向切换格式，每个格式列独立纵向滚动。
- 所有格式分组始终可见；素材数量悬殊时，各列也不会互相撑高或产生长距离空白。
- JPG/RAW 预览支持按钮、双击和触控板缩放，放大后可在预览区内移动查看细节。
...
6. 在右侧选择输出目录。应用默认“移动原片”；如需保留源目录，请切换为“复制原片”，再点击“导出已选”。

DJI 素材目录中若存在 `DJI_0001.MP4` 与 `DJI_0001.LRF`，程序会自动关联两者。也可以先单独导入 LRF，之后再导入视频。LRF 只用于快速预览和播放，不参与色彩格式判断，也不会作为独立素材显示；判断始终来自原视频元数据。

拖动窗口边缘或四角可以自由调整工作区大小；点击右上角的全屏图标、窗口左上角绿色按钮，或使用 macOS 标准全屏快捷键 `Control-Command-F` 可进入或退出全屏。
